...
def rhombus(rect):
    ...
    return [[x - 3 * w // 4, y], [x, y - h // 2], [x + 3 * w // 4, y],
            [x, y + h // 2]]


def parallelogram(rect):
    x, y, w, h = tuple(map(int, rect))
    return [[x - 3 * w // 4, y + h // 2], [x - w // 2, y - h // 2],
            [x + 3 * w // 4, y - h // 2], [x + w // 2, y + h // 2]]
...
def draw(obj, screen, x, y):
    ...
    if type(obj) in (StartNode, StopNode):
        pygame.draw.ellipse(
            screen, BLUE,
            pygame.Rect(x - NODEW / 2, y - NODEH / 2, NODEW, NODEH))
    elif type(obj) is Node:
        pygame.draw.rect(
            screen, NODE_COLOR,
            pygame.Rect(x - NODEW / 2, y - NODEH / 2, NODEW, NODEH))
    elif type(obj) in (InputNode, OutputNode):
        pygame.draw.polygon(screen, NODE_COLOR,
                            parallelogram((x, y, NODEW, NODEH)))
